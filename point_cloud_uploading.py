...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Upload point cloud files to a local server.")
    ...
    if device_id:
        ...
        # Define the path for the orbbec_recordings and point_cloud folders
        ...
        if os.path.exists(point_cloud_path):
            try:
                while True:
                    upload_files_in_folder(url, point_cloud_path, token, args.patient_id, device_id, proxies)
            except KeyboardInterrupt:
                print("\nLoop terminated by user.")
        else:
            print("point_cloud folder not found.")
    else:
