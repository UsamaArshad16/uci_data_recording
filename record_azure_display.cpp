    ...
    while (true) {
        ...
        if (result == K4A_WAIT_RESULT_SUCCEEDED) {
            ...
            if (color_image != NULL) {
                ...
                cv::resize(cv_image, resized_image, cv::Size(cv_image.cols / 2, cv_image.rows / 2));

                // Display the resized image in a window
                cv::imshow("RGB Stream", resized_image);
                cv::waitKey(1);  // Wait for 1 ms to allow display to refresh
